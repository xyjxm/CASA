...
class G1Deploy {
  private:
    ...
    bool quiet_;           ///< Reduce high-frequency console logging.
    int timing_log_interval_ticks_; ///< Control ticks between timing logs.
    int init_wait_log_counter_ = 0; ///< Counter for LowState wait log throttling.
  ...
  public:
    ...
    G1Deploy(
      ...
      bool quiet = false,
      double timing_log_interval_seconds = 1.0,
      double command_publish_frequency = 500.0)
      : time_(0.0),
        publish_dt_(1.0 / command_publish_frequency),
        control_dt_(0.02),
        planner_dt_(0.1),
        input_dt_(0.01),
        duration_(3.0),
        counter_(0),
        quiet_(quiet),
        timing_log_interval_ticks_(std::max(1, static_cast<int>(timing_log_interval_seconds / 0.02))),
        mode_pr_(Mode::PR),
        mode_machine_(0),
    ...
    void SetThreadPriority() {
      ...
      const char* main_cpu_env = std::getenv("G1_DEPLOY_MAIN_CPU");
      if (main_cpu_env == nullptr || main_cpu_env[0] == '\0') {
        return;
      }

      char* parse_end = nullptr;
      long main_cpu = std::strtol(main_cpu_env, &parse_end, 10);
      if (*parse_end != '\0' || main_cpu < 0 || main_cpu >= CPU_SETSIZE) {
        std::cerr << "Warning: ignoring invalid G1_DEPLOY_MAIN_CPU=" << main_cpu_env
                  << std::endl;
        return;
      }
      ...
      cpu_set_t cpuset;
      CPU_ZERO(&cpuset);
      CPU_SET(static_cast<int>(main_cpu), &cpuset);
    ...
    void Control() {
      ...
      switch (program_state_) {
        ...
        case ProgramState::CONTROL: {
          ...
          if (!quiet_ && logging_counter_ % timing_log_interval_ticks_ == 0) {
...
int main(int argc, char const* argv[]) {
  if (argc < 4) {
    ...
    std::cout << "  --domain-id <id>: Unitree DDS domain id for sim/deploy isolation (default: 0)" << std::endl;
    ...
    std::cout << "  --quiet: reduce high-frequency console logs" << std::endl;
    std::cout << "  --timing-log-interval-seconds <seconds>: timing log period when not quiet (default: 1)" << std::endl;
    std::cout << "  --command-publish-frequency <hz>: low command publish rate (default: 500; use 200 for sim)" << std::endl;
  ...
  bool quiet = false;
  double timingLogIntervalSeconds = 1.0;
  double commandPublishFrequency = 500.0;
  int zmq_out_port = 5557;
  std::string zmq_out_topic = "g1_debug";
  ...
  for (int i = 4; i < argc; i++) {
    if (std::string(argv[i]) == "--disable-crc-check") {
      ...
      if (i + 1 < argc) { domain_id = std::stoi(argv[i + 1]); i++; }
      ...
      quiet = true;
    } else if (std::string(argv[i]) == "--timing-log-interval-seconds") {
      if (i + 1 < argc) {
        try {
          timingLogIntervalSeconds = std::stod(argv[i + 1]);
          if (timingLogIntervalSeconds <= 0.0) {
            std::cerr << "Error: --timing-log-interval-seconds must be positive" << std::endl;
            exit(1);
          }
        } catch (...) {
          std::cerr << "Error: Invalid timing log interval value: " << argv[i + 1] << std::endl;
          exit(1);
        }
        i++;
      } else {
        std::cerr << "Error: --timing-log-interval-seconds requires a value argument" << std::endl;
        exit(1);
      }
    } else if (std::string(argv[i]) == "--command-publish-frequency") {
      if (i + 1 < argc) {
        try {
          commandPublishFrequency = std::stod(argv[i + 1]);
          if (commandPublishFrequency <= 0.0) {
            std::cerr << "Error: --command-publish-frequency must be positive" << std::endl;
            exit(1);
          }
        } catch (...) {
          std::cerr << "Error: Invalid command publish frequency value: " << argv[i + 1] << std::endl;
          exit(1);
        }
        i++;
      } else {
        std::cerr << "Error: --command-publish-frequency requires a value argument" << std::endl;
        exit(1);
      }
  ...
  G1Deploy custom(
    ...
    quiet,
    timingLogIntervalSeconds,
    commandPublishFrequency
  );
  if (!quiet) {
