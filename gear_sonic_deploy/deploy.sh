...
show_usage() {
    ...
    echo "  --domain-id ID          Set Unitree DDS domain id (default: 0)"
    echo "  --zmq-out-port PORT     Set ZMQ output port (default: 5557)"
    echo "  --quiet                 Reduce high-frequency deployment logs"
    echo "  --timing-log-interval-seconds N"
    echo "                           Timing log period when not quiet (default: 1)"
    echo "  --command-publish-frequency HZ"
    echo "                           Low command publish rate (default: 500; use 200 for sim)"
    echo "  --enable-csv-logs       Enable split CSV state logs"
    echo "  --logs-dir PATH         Directory for split CSV state logs"
    echo "  --record-input-file PATH"
    echo "                           Write input-state replay CSV"
    echo "  --target-motion-logfile PATH"
    echo "                           Write active target motion CSV"
    echo "  --policy-input-logfile PATH"
    echo "                           Write policy observation tensor CSV"
    echo ""
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --domain-id)
            if [[ -z "$2" ]]; then
                echo -e "${RED}Error: --domain-id requires an id argument${NC}" >&2
                exit 1
            fi
            DOMAIN_ID="$2"
            shift 2
        ...
        --zmq-out-port)
            ...
            ;;
        --quiet)
            DEPLOY_EXTRA_ARGS+=("--quiet")
            shift
            ;;
        --timing-log-interval-seconds)
            if [[ -z "$2" ]]; then
                echo -e "${RED}Error: --timing-log-interval-seconds requires a value argument${NC}" >&2
                exit 1
            fi
            DEPLOY_EXTRA_ARGS+=("--timing-log-interval-seconds" "$2")
            shift 2
            ;;
        --command-publish-frequency)
            if [[ -z "$2" ]]; then
                echo -e "${RED}Error: --command-publish-frequency requires a value argument${NC}" >&2
                exit 1
            fi
            DEPLOY_EXTRA_ARGS+=("--command-publish-frequency" "$2")
            shift 2
            ;;
        --enable-csv-logs)
            DEPLOY_EXTRA_ARGS+=("--enable-csv-logs")
            shift
            ;;
        --logs-dir)
            if [[ -z "$2" ]]; then
        ...
        --record-input-file)
            ...
            DEPLOY_EXTRA_ARGS+=("--record-input-file" "$2")
            shift 2
            ;;
        --target-motion-logfile)
            ...
            DEPLOY_EXTRA_ARGS+=("--target-motion-logfile" "$2")
            shift 2
            ;;
        --policy-input-logfile)
            ...
            DEPLOY_EXTRA_ARGS+=("--policy-input-logfile" "$2")
            shift 2
            ;;
        sim|real)
            INTERFACE_MODE="$1"
...
# Domain id and ZMQ output port are already set from argument parsing above
...
echo -e "${BLUE}    --domain-id $DOMAIN_ID \\${NC}"
echo -e "${BLUE}    --zmq-out-port $ZMQ_OUT_PORT${NC}"
if [[ -n "$EXTRA_ARGS" ]]; then
echo -e "${BLUE}    $EXTRA_ARGS${NC}"
fi
if [[ ${#DEPLOY_EXTRA_ARGS[@]} -gt 0 ]]; then
echo -e "${BLUE}    ${DEPLOY_EXTRA_ARGS[*]}${NC}"
fi
echo ""
echo -e "${CYAN}═══════════════════════════════════════════════════════════════════════${NC}"
echo ""
