...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--phase4-root", type=Path, default=Path("outputs/casa/phase4_dataset_v1_strict_50k_20260522"))
    parser.add_argument("--phase5-root", type=Path, default=Path("outputs/casa/phase5_conformal_baselines_20260522"))
...
def main() -> None:
    ...
    deploy_proc = None
    try:
        sim_proc = subprocess.Popen(_sim_cmd(args, live_sim_log, command_file, ack_file), cwd=REPO_ROOT, env=env, stdout=sim_log, stderr=subprocess.STDOUT)
        time.sleep(3.0)
        if sim_proc.poll() is not None:
            raise RuntimeError(f"sim exited early with code {sim_proc.returncode}; see {logs_dir / 'sim.log'}")

        deploy_proc = subprocess.Popen(_deploy_cmd(args, deploy_log_dir), cwd=DEPLOY_ROOT, env=env, stdout=deploy_log, stderr=subprocess.STDOUT)
        ...
        with online_log_path.open("w") as online_log:
            result = subprocess.run(online_cmd, cwd=REPO_ROOT, env=env, stdout=online_log, stderr=subprocess.STDOUT)
