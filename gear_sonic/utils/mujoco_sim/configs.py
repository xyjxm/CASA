...
class SimLoopConfig(BaseConfig):
    ...
    """Disable the elastic support band at startup for headless runs."""

    drop_after_seconds: Optional[float] = None
    """Disable the elastic support band after this many simulator seconds."""

    fall_log_interval_seconds: float = 5.0
    """Minimum seconds between repeated fall warning logs."""

    fall_stats: bool = True
    """Print fall statistics when the simulator exits."""

    disable_reset_on_fall: bool = False
    """Keep the robot fallen instead of automatically resetting after a fall."""

    sim_timing_log_interval_seconds: float = 5.0
    """Print simulator loop timing summary every N seconds. Set 0 to disable."""

    episode_log_dir: Optional[str] = None
    """Directory for Stage 1 episode CSV/JSON logs. Disabled when unset."""

    episode_log_fps: float = 50.0
    """Sampling rate for Stage 1 episode logs."""

    episode_name: str = "stage1_smoke"
    """Episode name written into Stage 1 episode summaries."""

    robot_scene: Optional[str] = None
    """Optional ROBOT_SCENE override, relative to repo root or absolute."""

    domain_id: Optional[int] = None
    """Optional Unitree DDS domain override for running isolated sim/deploy lanes."""

    casa_props_config: Optional[str] = None
    """Optional CASA scene props yaml; enables runtime prop placement."""

    casa_props_seed: int = 1234
    """Seed used for CASA scene prop placement at simulator startup."""

    casa_props_command_file: Optional[str] = None
    """Optional JSON command file for runtime CASA prop reset/randomize requests."""

    casa_props_ack_file: Optional[str] = None
    """Optional JSON ack file for runtime CASA prop commands."""

    enable_image_publish: bool = False
    """Enable image publishing in simulation"""

    image_publish_fps: Optional[float] = None
    """Offscreen image publish FPS. Defaults to the WBC IMAGE_DT setting."""

    camera_port: int = 5555
    """Camera port for image publishing"""

    offscreen_camera: Literal["track", "head_camera"] = "track"
    """Offscreen camera view to publish."""

    offscreen_camera_width: int = 960
    """Offscreen camera image width."""

    offscreen_camera_height: int = 720
    """Offscreen camera image height."""

    offscreen_camera_distance: float = 3.0
    """Tracking camera distance from the robot."""
