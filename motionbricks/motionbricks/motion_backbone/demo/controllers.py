...
class WASD_controller(base_controller):
    ...
    def generate_control_signals(self, camera_view, mj_model: mujoco.MjModel, mj_data: mujoco.MjData,
        ...
        movement_direction, facing_direction = \
            self._generate_target_position_and_heading(camera_view, mj_model, mj_data, key_pressed, mode)
    ...
    def _generate_target_position_and_heading(self, camera_view, mj_model: mujoco.MjModel,
        ...
        lookat_position = camera_view.cam.lookat

        cam_distance = camera_view.cam.distance
        cam_azimuth = -np.radians(camera_view.cam.azimuth) - np.pi / 2.0
        cam_elevation = -1 * np.radians(camera_view.cam.elevation)  # negative elevation means looking down
