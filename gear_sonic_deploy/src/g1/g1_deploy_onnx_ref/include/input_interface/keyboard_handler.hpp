  ...
  public:
    ...
    void handle_input(MotionDataReader& motion_reader,
      ...
      if (this->motion_prev && !motion_reader.motions.empty()) {
          ...
          std::cout << "Selected motion " << motion_reader.current_motion_index_
                    << ": " << motion_name << " (paused at frame 0)" << std::endl;
      }
      ...
      if (this->play_motion) {
          if (!operator_state.play) {
              ...
              std::string motion_name;
              {
                ...
                motion_name = current_motion ? current_motion->name : "";
              }
              std::cout << "Playing motion " << motion_reader.current_motion_index_
                        << ": " << motion_name << " from frame " << frame_copy
                        << " to end (" << timesteps_copy << " total frames)" << std::endl;
          }
      }
      ...
      if (this->motion_restart) {
          ...
          std::cout << "Reset motion " << motion_reader.current_motion_index_
                    << ": " << motion_name << " to frame 0 (paused)" << std::endl;
      }
